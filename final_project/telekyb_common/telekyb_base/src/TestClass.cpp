...
{
	intOp1 = addOption<int>("int1","Bla bla", 10);
	intOp2 = addBoundsOption<int>("int2","Bla bla", 20, 50, 5, true, true);
	doubleOp1 = addBoundsOption<double>("double", "BlaBla", 3.5, 2.0, 10.3);
	Eigen::Matrix3i defaultVal = Eigen::Matrix3i::Zero();
	defaultVal.diagonal() = Eigen::Vector3i(1,2,3);
	matOp3 = addOption<Eigen::Matrix3i>("matrix","test", defaultVal, false);
	levelVarOption = addOption< LevelValBaseEnum< const char * >::Type >("enum23","That's an enum", LevelVal::Debug);
	levelOption = addOption< LevelBaseEnum::Type >("enum","That's an enum", Level::Debug, false);
	pos3D =
			addOption<telekyb::Position3D>("pos3D","test", telekyb::Position3D(), false);
	...
	dynamicVec = addOption<Eigen::Matrix<double, Eigen::Dynamic, 2> >(
			"dynamicVec",
